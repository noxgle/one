...
## Project: Recover first-response reasoning-only stalls

### Goal

Ensure a tool-enabled turn does not terminate when llama.cpp with a reasoning
model such as `Qwen3.6-35B-A3B-MTP` emits non-empty `reasoning_content` but an
empty assistant `content` before making its first tool call. The immediate fix
must preserve the JSON-in-text contract and issue at most one recovery request.

### Context

Session `c5d7f45e-d11e-4cd2-a87c-2301b7e47188` was reported to display a
`Thought:` describing the requested implementation and then stop without a tool
call or `finish`. The private session is not required for reproduction and must
not be read. Repository tracing shows that llama.cpp uses the OpenAI-compatible
adapter, which emits `reasoning_content` through `thinking_delta` but returns an
empty `ChatResult.text`. `_should_tool_nudge()` currently rejects empty text,
post-tool repair is ineligible before the first tool result, and the empty
assistant message therefore ends the turn.

OpenCode comparison (source snapshot `18ef3cc`) shows that it normally sends
provider-native tool schemas and consumes normalized reasoning, text, and tool
calls as separate events. This handles empty textual content when a native tool
call is present. It does not universally recover when Qwen/llama.cpp emits only
reasoning with no recognized tool call; OpenCode issue `#24316` reports the same
class of Qwen tool markup appearing as thinking and stalling. Therefore the
bounded nudge below is the immediate compatibility fix, while native tool-call
normalization remains a separate architectural project.

### Scope

#### In Scope

- Track whether the current provider response emitted non-whitespace reasoning.
- Permit the existing step-0 nudge for reasoning-only, empty-content responses
  when tools are active and no tool result or normalized tool call exists.
- Preserve exactly one nudge attempt and the existing fallback when it does not
  produce a valid JSON-in-text tool call.
- Cover streaming and non-streaming OpenAI-compatible response shapes, with a
  llama.cpp/Qwen-compatible fake transport and no real provider access.
- Record the user-visible fix in the changelog and apply the required patch
  version/snapshot updates.

#### Non-Goals

- Parsing or executing tool syntax from `reasoning_content`.
- Merging reasoning into assistant text or persisted parseable content.
- Reading the reported private session or contacting a real provider in tests.
- Implementing provider-native tools in the immediate fix.
- Changing post-tool repair, retry, tool-step, abort, or budget semantics.

### Assumptions

- The displayed `Thought:` came from `reasoning_content`; the fake SSE fixture
  must reproduce this independently of the private session.
- A single strict nudge is acceptable additional latency for this failure path.
- The active tool set normally contains `finish`; disabled-tool behavior remains
  governed by the existing active-tool-aware nudge prompt.
- Existing uncommitted post-tool recovery changes on
  `fix/finish-after-repaired-tool` are retained and tested together, but they do
  not solve this first-response case.

### Open Questions

- Confirm whether the affected llama.cpp build emitted no native `tool_calls`,
  or emitted them in a response field currently ignored by `one`; this is not a
  blocker for the bounded nudge but informs the native-tools phase.
- Decide during native-tools design whether compatibility fallback should be
  per-provider, per-model, or negotiated after a rejected `tools` request.
- Determine policy for multiple native calls: reject, serialize, or support
  ordered execution. Do not decide implicitly in the immediate fix.

### Tech Stack

- **Python 3.12+:** agent loop and provider adapters.
- **httpx/SSE:** OpenAI-compatible streaming transport fixtures.
- **pytest:** fake-provider and fake-transport regression coverage.
- **llama.cpp OpenAI-compatible API:** affected local-provider surface.

### Constraints

- Reasoning must remain visible as `thinking_delta` but non-authoritative and
  non-parseable as a tool action.
- Recovery must be structurally bounded to one first-response nudge.
- No real MCP server, provider API, or private runtime state may be used by tests.
- Preserve existing event names, nudge counters, tool lifecycle ordering, and
  positive `maxSteps` behavior.
- Follow user-visible change policy: version bump, `CHANGELOG.md`, and regenerated
  TUI snapshots because the sidebar displays the version.

### Architecture

The OpenAI-compatible adapter continues to separate reasoning from assistant
content. `_invoke_provider()` records an ephemeral response-classification flag
such as `_hadThinking`, derived only from observed non-whitespace reasoning
deltas. The flag travels on the in-memory assistant response but is excluded
from provider-visible conversation flattening and persisted semantic content.

The agent loop evaluates the existing nudge once at step 0. Its predicate gains
an explicit reasoning-only branch: active tools, no prior tool result, empty
assistant text, non-empty reasoning observed, and no normalized provider tool
call. Normal short prose continues through the existing branch. A completely
empty response without reasoning remains on the explicit empty-response path.
The nudge response is parsed normally; if it still has no valid tool call, the
turn terminates without another request.

Longer term, provider results should expose normalized native calls
`{id, name, arguments}` separately from text and reasoning. OpenAI-compatible
requests would send tool schemas, assemble streamed call fragments, replay
assistant/tool messages with matching IDs, and retain JSON-in-text as an
explicit compatibility fallback during migration.

### Architecture Decisions

#### ADR-001: Use one reasoning-aware nudge as the immediate fix

**Decision:** Extend the existing step-0 nudge to one response that contains
reasoning but no assistant content or tool call.

**Alternatives:**

- Merge reasoning into parseable assistant content.
- Parse JSON/XML tool syntax directly from reasoning.
- Implement native provider tools before fixing the regression.
- Treat all empty responses as nudge candidates.

**Rationale:** The selected approach is small, preserves the security boundary
between reasoning and actions, directly addresses the observed llama.cpp/Qwen
shape, and remains bounded by the existing one-nudge control.

**Trade-offs:**

- Adds one provider request, latency, and token cost on affected turns.
- A model can still fail if the nudge also produces reasoning-only output.
- This is compatibility recovery rather than typed tool-call support.

**Consequences:** Reasoning remains display-only; successful recovery still
requires the model to return valid JSON-in-text on its second response.

#### ADR-002: Adopt OpenCode-style normalized native calls as a separate phase

**Decision:** Design native call normalization independently after the bounded
fix, while retaining JSON-in-text fallback until compatibility is demonstrated.

**Alternatives:**

- Keep JSON-in-text permanently as the sole contract.
- Replace JSON-in-text for every provider in one breaking migration.

**Rationale:** OpenCode's structured event approach avoids deriving actions from
prose and supports empty content with valid calls, but it requires coordinated
adapter, history, streaming, replay, event, and compatibility changes.

**Trade-offs:**

- Better typing and fewer repair requests versus substantial migration scope.
- Native llama.cpp behavior depends on server version, chat template, and tool
  parser configuration.

**Consequences:** The existing “Normalize provider-native tool calls” roadmap
item becomes an explicit follow-up rather than a prerequisite for this fix.

### Phases

#### Phase 1: Classify reasoning-only responses

**Objective:** Make the agent loop distinguish reasoning-only output from a
genuinely empty provider response without exposing reasoning to tool parsing.

**Prerequisites:** Existing OpenAI-compatible reasoning callbacks and nudge
events.

**Expected outcome:** An in-memory assistant response records whether meaningful
reasoning was observed while `content` remains unchanged.

**Estimated effort:** 0.5 day

**Confidence:** High

- [x] **Task:** Propagate a non-persisted reasoning-observed flag

  - **Description:** Track non-whitespace reasoning in `_invoke_provider()` for
    streaming and non-streaming adapters, attach the classification to the
    returned in-memory assistant message, and ensure flattening/persistence does
    not turn it into assistant text or provider-visible metadata.

  - **Files:** `one/core/agent_session.py`, `one/providers/openai_compatible.py`,
    `tests/test_providers.py`, relevant agent-session tests.

  - **Dependencies:** None.

  - **Acceptance Criteria:**
    - Reasoning-only responses retain empty assistant text.
    - Non-whitespace reasoning is distinguishable from a genuinely empty reply.
    - No reasoning text can be parsed or executed as a tool call.
    - Existing streaming `thinking_delta` output remains byte-for-byte unchanged.

  - **Verification:**
    - `.venv/bin/python -m pytest -q tests/test_providers.py`
    - Fake streaming and non-streaming fixtures assert empty text plus the
      response classification.

#### Phase 2: Recover once at the first tool boundary

**Objective:** Convert a first reasoning-only stall into one strict request for
a valid tool call, without introducing loops.

**Prerequisites:** Phase 1.

**Expected outcome:** The reported request proceeds from visible reasoning to a
tool call and ultimately `finish`, or terminates after one failed nudge.

**Estimated effort:** 0.5 day

**Confidence:** High

- [x] **Task:** Extend the bounded nudge predicate

  - **Description:** Add a reasoning-only eligibility branch to
    `_should_tool_nudge()` (or pass an explicit classification argument) while
    preserving current short-prose behavior. Require step 0, active tools, no
    prior tool result, empty content, reasoning observed, and no native/parsed
    call. Keep one nudge maximum.

  - **Files:** `one/core/agent_session.py`, `tests/test_agent_tool_calls.py`,
    optionally a dedicated OpenAI-compatible integration-style test module.

  - **Dependencies:** Phase 1 task.

  - **Acceptance Criteria:**
    - Reasoning-only first response triggers exactly one nudge.
    - A valid nudged JSON tool call executes once and can continue to `finish`.
    - A second reasoning-only or empty response causes no third provider call.
    - A genuinely empty first response without reasoning is not misclassified.
    - Normal prose, direct tool calls, post-tool repair, cancellation, budgets,
      and positive `maxSteps` retain their existing behavior.

  - **Verification:**
    - `.venv/bin/python -m pytest -q tests/test_agent_tool_calls.py`
    - Assert one `tool_call_nudge_start`/`end` pair, conversion counters, tool
      order, final summary, provider-call count, and bounded failure fallback.

- [x] **Task:** Add a llama.cpp/Qwen-compatible end-to-end fixture

  - **Description:** Drive the real `OpenAICompatibleAdapter` with fake SSE and
    non-streaming responses shaped as `reasoning_content` plus empty/null
    `content`, followed by JSON-in-text from the nudge. Include a control where
    the second response remains reasoning-only.

  - **Files:** `tests/test_providers.py`, `tests/test_agent_tool_calls.py` or a
    new focused test file under `tests/`.

  - **Dependencies:** Phase 1 task; bounded nudge task.

  - **Acceptance Criteria:**
    - The fixture reproduces the original stall before the behavior change.
    - Streaming and non-streaming paths recover consistently.
    - Tests use no real server, credentials, private sessions, or ambient state.

  - **Verification:**
    - Focused pytest invocation for the new regression test names.

#### Phase 3: Release and regression validation

**Objective:** Validate the combined first-response and post-tool recovery
behavior and prepare a user-visible patch release.

**Prerequisites:** Phases 1–2 and the uncommitted post-tool recovery fix.

**Expected outcome:** All recovery paths are bounded, independently tested, and
documented in a patch release.

**Estimated effort:** 0.5 day

**Confidence:** High

- [x] **Task:** Complete release metadata and full verification

  - **Description:** Add an `Unreleased` changelog entry describing
    reasoning-only first-response recovery, select the next patch version after
    confirming branch state, regenerate TUI snapshots, and run all project gates.

  - **Files:** `CHANGELOG.md`, `one/config.py`,
    `tests/snapshots/tui/base.txt`, `tests/snapshots/tui/overlay.txt`,
    `tests/snapshots/tui/widget_panel.txt`.

  - **Dependencies:** All Phase 2 tasks.

  - **Acceptance Criteria:**
    - Version and all displayed-version snapshots agree.
    - Changelog distinguishes first-response nudge from post-tool repair.
    - No unrelated snapshot or event-contract drift appears.
...
#### Phase 4: Normalize provider-native tool calls

**Objective:** Match OpenCode's typed tool-call architecture without dropping
compatibility with local servers that only satisfy the JSON-in-text contract.

**Prerequisites:** Phases 1–3; separate approval of the migration design.

**Expected outcome:** Empty assistant text with valid native calls executes
without a nudge; textual fallback remains deterministic and non-duplicating.

**Estimated effort:** 3–5 days

**Confidence:** Medium

- [ ] **Task:** Design and implement normalized native call transport

  - **Description:** Extend provider results with normalized call ID, name, and
    argument payload; send OpenAI-compatible tool schemas; assemble streamed
    fragments; validate arguments; replay matching assistant/tool messages; and
    define precedence when native and textual calls coexist. Keep JSON-in-text
    fallback until provider compatibility fixtures pass.

  - **Files:** `one/providers/base.py`, `one/providers/openai_compatible.py`,
    `one/core/agent_session.py`, `one/tools/index.py`, provider and event tests;
    other adapters are follow-up files only after the common contract is proven.

  - **Dependencies:** Separate ADR approval; llama.cpp compatibility matrix.

  - **Acceptance Criteria:**
    - Native calls execute independently of textual content and finish reason.
    - Streamed and non-streaming calls normalize identically.
    - Native calls take precedence over text without duplicate execution.
    - Unknown tools, malformed arguments, missing/duplicate IDs, and multiple
      calls have explicit deterministic handling.
    - JSON-in-text continues working for unsupported local servers.

  - **Verification:**
    - Wire tests for schemas and tool-choice policy.
    - Adversarial SSE fragmentation and duplicate-call tests.
    - Replay tests with matching `tool_call_id` values.
    - Separately authorized real llama.cpp/Qwen spot check against a pinned
      server build and documented chat-template/tool-parser configuration.

### Rollout & Rollback

Ship Phases 1–3 as one patch release with no configuration migration. Monitor
nudge fire/conversion counters by provider/model where available. Roll back the
reasoning-only eligibility branch if it causes excess nudges; the prior empty
response behavior then returns without affecting persisted sessions. Gate Phase
4 behind compatibility tests and retain JSON-in-text fallback throughout its
rollout.

### Observability

- Reuse `tool_call_nudge_start`/`tool_call_nudge_end` and existing fire/conversion
  counters; add a non-sensitive reason/classification field only if event
  compatibility permits it.
- Do not log reasoning text, credentials, private session content, or raw tool
  arguments solely for this diagnostic.
- Compare reasoning-only nudge conversion and failure rates by provider/model in
  separately authorized manual testing.

### Security Considerations

- Never parse or execute `reasoning_content` as a tool action.
- Only a normal, validated tool call may reach dispatch.
- Preserve untrusted tool-output boundaries and active-tool validation.
- Tests must use fake transports and in-memory session/auth storage.

### Risks & Mitigations

| Risk | Impact | Likelihood | Mitigation |
|------|--------|------------|------------|
| Recovery loops | High | Low | Structural one-nudge limit; assert provider-call count |
| Legitimate reasoning-only answer is nudged | Medium | Medium | Require active tools, step 0, empty content, and observed reasoning |
| Extra latency/token use | Medium | Medium | Trigger only on the narrow failure shape; monitor counters |
| Empty assistant replay rejected by provider | Medium | Low | Fake wire tests; avoid replaying invalid empty content if adapter requires omission |
| Reasoning accidentally becomes executable | High | Low | Keep reasoning out of parseable content and assert this invariant |
| llama.cpp lacks native tool support | Medium | Medium | Immediate JSON-in-text recovery; retain fallback in Phase 4 |
| Native/text duplicate execution | High | Medium | Define native precedence and deduplicate by call ID before Phase 4 rollout |

### Project Acceptance Criteria

- [x] A first streamed or non-streaming reasoning-only response with active tools
      receives exactly one strict nudge.
- [x] A valid nudged call can create the requested file and later execute
      `finish`; a failed nudge cannot loop.
- [x] Reasoning remains display-only and absent from tool parsing.
- [x] Completely empty, normal prose, direct-tool, post-tool repair, cancellation,
      budget, and `maxSteps` controls remain covered.
- [x] No private session/provider data is read and no real external service is
      contacted by automated tests.
- [x] Full pytest, Ruff, and diff-check gates pass.

### Estimated Timeline

Phases 1–3: approximately 1–1.5 engineering days including review and full
verification. Phase 4: 3–5 additional days after a separate architecture review,
with uncertainty dominated by llama.cpp versions, chat templates, streamed call
fragmentation, and cross-provider replay compatibility.
